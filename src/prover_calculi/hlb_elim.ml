...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let iter_ctx a b k =
    ...
    let rec aux a b =
      ...
      | T.App (hda, argsa), T.App (hdb, argsb) when Type.equal (T.ty a) (T.ty b)
        ->
  ...
  let register_cl_term cl premise =
    ...
    in
    cl_occs :=
      Util.Int_map.add (C.id cl)
        (Term.Set.add premise premises, propagated)
        !cl_occs
  ...
  let register_propagated_lit ~prop_kind lit_t cl cs =
    ...
    |> Iter.iter (fun (t, _, subst) ->
           if Subst.is_renaming subst then
             ...
             to_remove := Term.Set.add t !to_remove);
    ...
    if not !has_renaming then (
      ...
      propagated_ :=
        PropagatedLitsIdx.add !propagated_ lit_t (proofset, prop_kind);
      ...
      (fun tasks (t, (tbl, _), subst) ->
        let sets_to_remove =
          Iter.fold
            ...
            [] (T.Tbl.to_iter tbl)
        in
        if CCList.is_empty sets_to_remove then
          tasks
        else
  ...
  let normalize_variables premise concl =
    ...
    if T.VarSet.is_empty to_rename then
      premise, concl
    else (
      let renamer =
        T.VarSet.fold
  ...
  let do_unit_hle_htr cl =
    ...
    try
      ...
      if CCBV.is_empty (CCBV.negate bv) then
        None
      else (
        let lit_l = List.rev @@ CCBV.select bv (C.lits cl) in
        let proof =
          Proof.Step.simp
            ~rule:(Proof.Rule.mk "unit_hle/fle")
  ...
  let do_hte_hle cl =
    ...
    try
      ...
      if CCBV.is_empty (CCBV.negate bv) then
        ...
        let proof =
          Proof.Step.simp
            ~rule:(Proof.Rule.mk "hidden_literal_elimination")
            (List.map C.proof_parent (cl :: CS.to_list !proofset))
        in
        let res =
          C.create ~penalty:(C.penalty cl) ~trail:(C.trail cl) lit_l proof
    ...
    with
    | HiddenTauto (i, j, proofset) ->
      let lit_l = [ CCArray.get (C.lits cl) i; CCArray.get (C.lits cl) j ] in
      let proof =
        Proof.Step.simp
          ~rule:(Proof.Rule.mk "hidden_tautology_elimination")
          (List.map C.proof_parent (cl :: CS.to_list proofset))
      in
      let repl =
        C.create ~penalty:(C.penalty cl) ~trail:(C.trail cl) lit_l proof
  ...
  let do_context_simplification cl =
    ...
    CCArray.iteri
        ...
        if CCBV.get lits_to_keep i && not (Lit.is_predicate_lit lit) then (
          try
            CCArray.iteri
              (fun j lit' ->
                if
                  i != j && CCBV.get lits_to_keep j
                  && (not (Lit.is_predicate_lit lit'))
                  && Lit.is_positivoid lit = Lit.is_positivoid lit'
                then (
                  match Lit.View.as_eqn lit, Lit.View.as_eqn lit' with
                  | Some (s, t, sign), Some (ctx_s, ctx_t, _) ->
                    iter_ctx ctx_s ctx_t (fun (s', t') ->
                        if
                          (T.equal s s' && T.equal t t')
                          || (T.equal s t' && T.equal t s')
                        then
                          if sign then (
      ...
      (C.lits cl);
    if CCBV.is_empty (CCBV.negate lits_to_keep) then
      None
    else (
  ...
  let untrack_clause cl =
    ...
    | Some (premises, propagated) ->
      Term.Set.iter
        (fun premise ->
          prems_ :=
            PremiseIdx.update_leaf !prems_ premise (fun (tbl, _) ->
                T.Tbl.filter_map_inplace
                  (fun concl proofset ->
                    if CS.mem cl proofset then (
      ...
      Term.Set.iter
        (fun prop_lit ->
          propagated_ :=
            PropagatedLitsIdx.update_leaf !propagated_ prop_lit (fun (cs, _) ->
                if not @@ CS.mem cl cs then (
                  decr propagated_size_;
                  false
                ) else
                  true))
