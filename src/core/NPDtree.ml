...
let[@inline] mk_neg f = T.app neg_const [ f ]
...
let rec open_term ~stack t =
  ...
  | _ when (not (Unif.Ty.type_is_unifiable (T.ty t))) || Type.is_fun (T.ty t) ->
    Some { cur_term = t; stack = [] :: stack }
    (* opaque constant/partial application *)
  | T.App (f, _) when T.is_var f ->
    Some { cur_term = t; stack = [] :: stack } (* higher-order term *)
  | T.App (_, l) -> Some { cur_term = t; stack = l :: stack }
...
let rec next_rec stack =
  match stack with
  | [] -> None
  | [] :: stack' -> next_rec stack'
  | (t :: next') :: stack' -> open_term ~stack:(next' :: stack') t
...
module Make (E : Index.EQUATION) = struct
  ...
  let goto_leaf trie t k =
    ...
    let rec goto trie iter rebuild =
      match iter with
      | None ->
        (match k trie.leaf with
        | leaf' when leaf' == trie.leaf ->
          root (* no change, return same tree *)
        | leaf' -> rebuild { trie with leaf = leaf' })
      | Some i ->
        (match view_head i.cur_term with
        | As_star ->
          let subtrie =
            match trie.star with
            | None -> empty ()
            | Some trie' -> trie'
          in
          let rebuild subtrie =
            if is_empty subtrie then
              rebuild { trie with star = None }
            else
              rebuild { trie with star = Some subtrie }
          in
  ...
  let retrieve ?(subst = S.empty) ~sign dt t k =
    ...
    let rec traverse trie iter =
      match iter with
      | None -> Leaf.fold_match ~subst (Scoped.set dt trie.leaf) t k'
      | Some i ->
        (match view_head i.cur_term with
        | As_star ->
          (match trie.star with
          | None -> ()
          | Some subtrie -> traverse subtrie (next i))
          (* match "*" against "*" *)
        | As_app (s, _) ->
          (try
          ...
          (match trie.star with
          | None -> ()
          | Some subtrie -> traverse subtrie (skip i))
          (* skip subterm *))
    in
...
module MakeTerm (X : Set.OrderedType) = struct
    ...
    let rec goto trie iter rebuild =
      ...
      | Some i ->
        ...
        | As_app (s, l) ->
          ...
          let subtrie =
            try find_sub trie.map (s, arity) with NoSuchTrie -> empty ()
          in
          let rebuild subtrie =
            if is_empty subtrie then
              rebuild { trie with map = SIMap.remove (s, arity) trie.map }
            else
              rebuild { trie with map = SIMap.add (s, arity) subtrie trie.map }
          in
          goto subtrie (next i) rebuild)
    in
    goto trie (iterate t) (fun t -> t)
  ...
  let retrieve_unifiables_aux fold_unify dt t k =
    ...
    let rec traverse trie iter =
      match iter with
      | None -> fold_unify (Scoped.set dt trie.leaf) t k
      | Some i ->
        (match view_head i.cur_term with
        | As_star ->
          (* skip one term in all branches of the trie *)
          skip_tree trie (fun subtrie -> traverse subtrie (next i))
        | As_app (s, l) ->
          let arity = List.length l in
          (try
  ...
  let retrieve_specializations ?(subst = S.empty) dt t k =
    ...
    let rec traverse trie iter =
      match iter with
      | None -> Leaf.fold_matched ~subst (Scoped.set dt trie.leaf) t k
      | Some i ->
        (match view_head i.cur_term with
        | As_star ->
