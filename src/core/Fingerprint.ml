...
let expand_otf_ body =
  ...
  if CCList.is_empty extra_args then
    body
  else (
...
module Make (X : Set.OrderedType) = struct
  ...
  let add idx t data =
    ...
    let rec recurse trie features =
      ...
      | Node map, f :: features' ->
        let subtrie = try FeatureMap.find f map with Not_found -> Empty in
        (* insert in subtrie *)
  ...
  let remove_if idx t leaf_cleaner =
    ...
    let rec recurse trie features =
      match trie, features with
      | Empty, [] | Empty, _ :: _ -> Empty (* keep it empty *)
      | Node map, f :: features' ->
        let map =
          ...
          try
            ...
            let subtrie = recurse subtrie features' in
            if subtrie = Empty then
              FeatureMap.remove f map
            else
              FeatureMap.add f subtrie map
          with Not_found -> map
        in
        (* if the map is empty, use Empty *)
        if FeatureMap.is_empty map then
          Empty
        else
          Node map
      | Leaf leaf, [] ->
        let leaf = leaf_cleaner leaf t in
        if Leaf.is_empty leaf then
          Empty
        else
          Leaf leaf
      | Node _, [] | Leaf _, _ :: _ ->
  ...
  let fold idx f acc =
    let rec fold trie f acc =
      match trie with
      | Empty -> acc
      | Node map ->
        FeatureMap.fold (fun _ subtrie acc -> fold subtrie f acc) map acc
  ...
  let traverse ~compatible idx features k =
    ...
    let rec recurse trie features =
      match trie, features with
      | Empty, _ -> ()
      | Leaf leaf, [] -> k leaf (* give the leaf to [k] *)
      | Node map, f :: features' ->
