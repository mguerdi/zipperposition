...
let rec all_combs = function
  ...
  | x :: xs ->
    ...
    if CCList.is_empty rest_combs then
      CCList.map (fun t -> [ t ]) x
    else
      CCList.flat_map (fun i -> CCList.map (fun comb -> i :: comb) rest_combs) x
...
let rec size t =
  ...
  | AppBuiltin (_, l) | App (_, l) ->
    List.fold_left (fun s t' -> s + size t') 1 l
  | Fun (_, u) -> 1 + size u
  | Const _ -> 1
...
let rec in_pfho_fragment t =
  ...
  | Var _ ->
    if not (type_ok (ty t)) then
      ...
      true
  | Const sym ->
    if top_level_exception t || type_ok (ty t) then
      true
    else
      raise
        (Failure
      ...
      raise
        (Failure
           (CCFormat.sprintf
              ...
              Type.pp (ty t)))
  | Fun (var_t, body) ->
    if type_ok var_t && type_ok (ty body) && in_pfho_fragment body then
      true
    else
      raise
        (Failure
           (CCFormat.sprintf "Lambda body has out-of-fragment type [%a]" T.pp t))
  | DB _ ->
    if type_ok (ty t) then
      true
    else
      raise (Failure "Bound variable has out-of-fragment type")
...
and top_level_exception t =
  ...
  if is_var hd || hd_is_skolem then
    false
  else if Type.equal (ty t) Type.prop then
    true
  else
    false
...
let in_lfho_fragment t =
  ...
  && Seq.subterms t |> fun subts ->
     if Iter.for_all (fun subt -> not (is_fun subt)) subts then
       true
     else
       raise (Failure "Term contains a lambda")
...
let max_d_l =
  let rec max_d_l_aux acc = function
    | [] -> acc
    | x :: xs -> max_d_l_aux (max_d x acc) xs
  in
  max_d_l_aux None
    ...
    let rec normalize t =
      ...
      | T.App (f, l)
        ...
        | [ a; b ] ->
          ...
          else if T.equal a a' && T.equal b b' then
            t
          else
            T.app ~ty:(ty t :> T.t) f (tyargs @ [ a'; b' ])
...
module DB = struct
  ...
  let unskolemize sk_to_vars t =
    let rec aux depth subt =
      ...
      | None ->
        ...
        | AppBuiltin (hd, l) ->
          app_builtin ~ty:(ty subt) hd (List.map (aux depth) l))
    in
    aux 0 t
  ...
  let rec map_vars_shift ?(depth = 0) var_map t =
    ...
    | AppBuiltin (hd, l) ->
      app_builtin ~ty:(ty t) hd (List.map (map_vars_shift ~depth var_map) l)
end
  ...
  let pp_depth ?hooks:_ depth out t =
    ...
    and pp_enclosed out t =
      if Type.is_tType (ty t) then (
        ...
        match view t with
        | App _ | AppBuiltin _ -> Format.fprintf out "(@[%a@])" pp_rec t
        | _ -> pp_rec out t
      )
    in
    pp_rec out t
    ...
    let rec aux t =
      ...
      | PT.Bind (b, v, body)
        ...
        if TypedSTerm.Ty.is_tType (Var.ty v) then
          ...
          let ty_arg = Type.Conv.of_simple_term_exn ctx (Var.ty v) in
          let previous =
            if PT.Var_tbl.mem tbl v then
              Some (PT.Var_tbl.find tbl v)
            else
              None
          in
      ...
      | PT.Meta _ | PT.Record _ | PT.Ite _ | PT.Let _ | PT.Match _
      | PT.Multiset _ | _ ->
...
let rebuild_rec t =
  let rec aux env t =
    ...
    | DB i ->
      ...
      assert (
        if Type.equal ty (List.nth env i) then
          true
        else (
...
let rec normalize_bools t =
  ...
  | AppBuiltin
    ...
    else if T.equal x x' && T.equal y y' then
      t
    else
      app_builtin ~ty:Type.prop b l
  | AppBuiltin (hd, l) ->
    let l' = List.map normalize_bools l in
    if same_l l' l then
      t
    else
      app_builtin ~ty:(ty t) hd l'
