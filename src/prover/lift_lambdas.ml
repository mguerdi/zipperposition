...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  module Idx = Fingerprint.Make (struct
    ...
    let ( <?> ) = CCOrd.Infix.( <?> )
    let compare (a, b) (c, d) = C.compare a c <?> (T.compare, b, d)
  end)
      ...
      | DB i ->
        if i < depth then
          ...
          | Some var ->
            assert (
              not
              @@ Scoped.equal T.equal (sc var) (Subst.FO.deref subst (sc var)));
            var, lb_map, subst
          | None ->
  ...
  let fully_apply ~counter s t =
    ...
    if not (Type.is_fun (T.ty t)) then
      s, t
    else (
  ...
  let lift_lambdas cl =
    ...
    let lits, (reused_defs, new_defs) =
      C.lits cl |> CCArray.to_list |> fun l ->
      List.fold_right
